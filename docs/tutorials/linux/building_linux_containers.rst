...
* ``RUN``: these describe how to install software onto the machine. Because Docker creates layers for each RUN directive, you should use "&" to group related commands together and minimize the number of layers (thus disk space and bandwith).
...
If no command is provided, then docker will run the final command in the contain's configuration.
